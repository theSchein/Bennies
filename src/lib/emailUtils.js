...
const generateOnboardEmailHtml = (data, username, collectionName, collectionUtility, collectionLink) => {
    return `
  <!DOCTYPE html>
  <html>
  <head>
    <style>
      ...
      .button { background-color: #294B29; color: #E1F0DA; padding: 10px 20px; text-decoration: none; border-radius: 5px; border: none; outline: none; }
      .button:hover { background-color: #50623A; }
      .attributes { margin: 20px 0; }
      .attributes_content { background-color: #D4E7C5; padding: 10px; border-radius: 5px; }
      ...
      .social-links a { margin: 0 10px; color: #294B29; text-decoration: none; }
  ...
  <body>
    ...
    <div class="content">
      <h1>Hi ${username}!</h1>
      <p>Welcome to the ${collectionName} community! We are thrilled to have you with us. Here's what you can expect as a new member:</p>
      
      <h2>Listing</h2>
      <div class="body-action">
        <a href="${collectionLink}" class="button" target="_blank">${collectionName}</a>
      </div>
      <p>Thank you for being a part of ${collectionName}. We can't wait to see what you'll achieve in our community.</p>
      
      ${data.goal ? `<div class="attributes"><h3>Goal</h3><div class="attributes_content">${data.goal}</div></div>` : ''}
      
      <div class="social-links">
        ${data.twitter_link ? `<a href="${data.twitter_link}" target="_blank"><img src="https://example.com/twitter-icon.png" alt="Twitter"></a>` : ''}
        ${data.discord_link ? `<a href="${data.discord_link}" target="_blank"><img src="https://example.com/discord-icon.png" alt="Discord"></a>` : ''}
        ${data.telegram_link ? `<a href="${data.telegram_link}" target="_blank"><img src="https://example.com/telegram-icon.png" alt="Telegram"></a>` : ''}
      </div>
...
export const sendOnboardingEmail = async (email, username, collectionName, collectionLink, emailData) => {
